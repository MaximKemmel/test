...
export default function ConnectionPopup({referralId, referralAddress}: RegistrationPopupProps) {
    const {isOpen, close} = usePopupConnect();
                    ...
                    .then(() => allowanceUSDT(allowanceOptions).then(() => allowanceDP1(allowanceOptions).then(async () => {
                            ...
                            if (referral_all !== '1') {
                                ...
                                if (referrer.length > 8) {
                                    user = await getUserByAddress(referrer)

                                } else {
                                    user = await getUserById(referrer)
                                }
